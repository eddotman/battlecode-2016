...
public class RobotPlayer {
  ...
  public static MapLocation findParts(MapLocation rLoc, RobotType rType, RobotController rc) {
    int maxRad = (int)(rType.sensorRadiusSquared + 1);
        ...
        try {
          ...
          if (rc.isCoreReady()) {
            ...
            if (rc.hasBuildRequirements(typeToBuild)) {
              ...
              if (partsLoc != myLoc) {
                if (rc.canMove(partsDir)){
                  rc.move(partsDir);
                } else if (rc.senseRubble(myLoc.add(partsDir)) > GameConstants.RUBBLE_OBSTRUCTION_THRESH) {
                  rc.clearRubble(partsDir);
                } else {
                  Clock.yield();
                }
